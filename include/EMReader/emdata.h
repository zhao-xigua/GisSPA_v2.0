...
class Euler {
  ...
  void setAngle(float e1, float e2, float e3, float e0,
                int typ);   // quaternion, spin, and SGIROT setAngle
  ...
  float alt_mrc, az_mrc,
      phi_mrc;  // mrc euler angle, before using them, need to call convertToMRCAngle()
  float m_[9];
...
struct emdataHeader {
  ...
  float xorigin, yorigin,
      zorigin;  // the origin coordinates in Angstrom for the first pixel (0,0,0).
...
class emdata {
  ...
  emdata* project3d(emdata* proj, float alt, float az, float phi, int mode, float thr = 1.0, int ri = 0,
                    int dcx = 0, int dcy = 0, int dcz = 0);
  ...
  inline float trilin(float p1, float p2, float p3, float p4, float p5, float p6, float p7,
                      float p8, float t, float u, float v) {
    return (1.0 - t) * (1.0 - u) * (1.0 - v) * p1 + t * (1.0 - u) * (1.0 - v) * p2 +
           (1.0 - t) * u * (1.0 - v) * p3 + t * u * (1.0 - v) * p4 +
           (1.0 - t) * (1.0 - u) * v * p5 + t * (1.0 - u) * v * p6 + (1.0 - t) * u * v * p7 +
           t * u * v * p8;
  }
  ...
  inline void setName(const char* n) {
    strncpy(header.name, n, 79);
    header.name[79] = 0;
  }
  inline void setPath(const char* p) {
    strncpy(header.path, p, MAXPATHLEN - 1);
