...
void emdata::zero(float sig) {
  if (sig == 0)
    memset(rdata, 0, header.nx * header.ny * header.nz * sizeof(float));
  else {
    int i;
...
int emdata::readHDF(const char* fsp, int image_index, int nodata) {
  ...
  header.particle_location_center_x =
      hdf_file.read_float_attr(image_index, "particle_location_center_x");
  header.particle_location_center_y =
      hdf_file.read_float_attr(image_index, "particle_location_center_y");

  float orientation[4];
  header.orientation_convention = static_cast<Euler::EulerType>(
      hdf_file.read_euler_attr(image_index, "orientation_convention"));
  ...
  switch (header.orientation_convention) {
    ...
    case Euler::QUAT:
    case Euler::SPIN:
      euler = Euler(orientation[0], orientation[1], orientation[2], orientation[3],
                    header.orientation_convention);
      break;
  ...
  if (hdf_file.read_ctfit(image_index, ctfit) != 1) {
    ...
    sprintf(buf, "!-%1.3g %1.3g %1.3g %1.3g %1.3g %1.3g %1.3g %1.3g %1.3g %1.3g %1.3g",
            header.ctf[0], header.ctf[1], header.ctf[2], header.ctf[3], header.ctf[4],
            header.ctf[5], header.ctf[6], header.ctf[7], header.ctf[8], header.ctf[9],
            header.ctf[10]);
...
int emdata::readMRC(const char* fsp, int nodata, int n) {
  ...
  if (mrch->nz == 1)
    sprintf(s, "Read a %s %s %dx%d 2D %s MRC File\n", str2[ord + mord], str3[pipe], nx, ny,
            str[mrch->mode]);
  else
    sprintf(s, "Read a %dx%dx%d 3D MRC File\n", nx, ny, nz);
  ...
  header.pixel =
      mrch->xlen / (float)(nx);  // somebody changed the writeMRC to use nx instead of nx-1, so this
                                 // is update to use nx too for self consistence
...
void emdata::rotate(float e) {
  ...
  setXYZOrigin(header.xorigin - header.dx * header.pixel, header.yorigin - header.dy * header.pixel,
...
EMObject::operator std::vector<float>() const {
  if (type != FLOATARRAY) {
    if (type != UNKNOWN) {
...
EMObject::operator std::vector<std::string>() const {
  if (type != STRINGARRAY) {
    if (type != UNKNOWN) {
...
int emdata::writeImage(const char* filename, int n,
                       int headonly) {  // convnience function, it will actually call the following
                                        // functions by guessing image types
  int flen = strlen(filename);
  int r = 0;  // success =0. fail = 1
  if (flen > 4 && strcasecmp(&filename[flen - 4], ".hdf") == 0)
    r = writeHDF2(filename, n, headonly);
  else {
    fprintf(stderr,
            "WARNING: cannot determine the intented image format for file name\"%s\", assuming "
            "HDF5 format!\n",
            filename);
    r = writeHDF2(filename, n, headonly);
  }
...
int emdata::writeHDF2(const char* filename, int n, int headonly) {
  const char* const rw_mode = "rw";

  EMhdf2* hdf_file = new EMhdf2(filename, rw_mode);
  int image_index = n;
  if (image_index < 0) {
    image_index = hdf_file->get_num_dataset();
  }
  ...
  header.nz = nz;
  header.pixel = pixel;
  header.xorigin = xorigin;
  header.yorigin = yorigin;
  header.zorigin = zorigin;
  header.min = min;
  header.max = max;
  header.mean = mean;
  header.sigma = sigma;
  setMicrographId(header.micrograph_id);
  header.particle_location_center_x = particle_location_center_x;
  header.particle_location_center_y = particle_location_center_y;
  header.center_x = center_x;
  header.center_y = center_y;
  header.alipeak = alipeak;
  header.nimg = nimg;
  for (int i = 0; i < 11; ++i) {
    header.ctf[i] = ctf[i];
  }
  header.orientation_convention = orientation_convention;
  header.map_type = map_type;

  hdf_file->write_header(header, image_index, this->euler);

  if (!headonly) {
    hdf_file->write_data(rdata, image_index);
  }
...
emdata* emdata::project3d(emdata* proj, float alt, float az, float phi, int mode, float thr, int ri,
                          int dcx, int dcy, int dcz) {
  // this is essentially identical to mode=-4. but this mode accumulate the results
  // directly in the 2D image instead of storing in the another rotated copy then
  // accumulating (ie: less memory requirement) this mode does not modify the self data
  // either in this mode ri is used as mask radius, emdata variable scale is used as
  // scale, emdata variable dx and dy are used as translation for the 2-D projection
  ...
  nx = header.nx;
  ny = header.ny;
  // printf("emdata::project3d(): mode = -6\n");
  float z2, u, v, t;
  int l = 0, xy;
  int rmax;
  rmax = int(ri * scale + 0.5);  // proj = scale * map
  if (rmax <= 0) {
    rmax = nx / 2;
  }
  float r2 = rmax * rmax;
  ...
  mx[0] = (cos(phi) * cos(az) - cos(alt) * sin(az) * sin(phi)) / scale;
  mx[1] = -(sin(phi) * cos(az) + cos(alt) * sin(az) * cos(phi)) / scale;
  mx[2] = sin(alt) * sin(az) / scale;
  mx[3] = (cos(phi) * sin(az) + cos(alt) * cos(az) * sin(phi)) / scale;
  mx[4] = (-sin(phi) * sin(az) + cos(alt) * cos(az) * cos(phi)) / scale;
  mx[5] = -sin(alt) * cos(az) / scale;
  mx[6] = sin(alt) * sin(phi) / scale;
  mx[7] = sin(alt) * cos(phi) / scale;
  mx[8] = cos(alt) / scale;
  // std::cout << "mx[0] = " << mx[0] << std::endl;
  // std::cout << "mx[1] = " << mx[1] << std::endl;
  // std::cout << "mx[2] = " << mx[2] << std::endl;
  // std::cout << "mx[3] = " << mx[3] << std::endl;
  // std::cout << "mx[4] = " << mx[4] << std::endl;
  // std::cout << "mx[5] = " << mx[5] << std::endl;
  // std::cout << "mx[6] = " << mx[6] << std::endl;
  // std::cout << "mx[7] = " << mx[7] << std::endl;
  // std::cout << "mx[8] = " << mx[8] << std::endl;
  xy = nx * ny;
  int jmin = int(-rmax + dy + 0.5), jmax = int(rmax + dy + 0.5);
  int imin = int(-rmax + dx + 0.5), imax = int(rmax + dx + 0.5);
  if (jmin < -ny / 2) jmin = -ny / 2;
  if (jmax > ny / 2) jmax = ny / 2;
  if (imin < -nx / 2) imin = -nx / 2;
  if (imax > nx / 2) imax = nx / 2;
  // std::cout << "imax=" << imax << std::endl;
  // std::cout << "jmax=" << jmax << std::endl;
  proj->setSize(nx, ny);
  proj->zero();
  float* sdata = getData();
  float* ddata = proj->getData();
  // std::cout << "sdata address: " << sdata << std::endl;
  // std::cout << "ddata address: " << ddata << std::endl;
  for (int k = -rmax; k < rmax; k++) {
    ...
    for (int j = jmin; j < jmax; j++) {
      ...
      for (int i = imin; i < imax; i++) {
        float ix = i - dx;
        if (r2 < (ix * ix + jy * jy + kz * kz)) continue;

        x2 = mx[0] * ix + mx[1] * jy + mx[2] * kz + nx / 2;
        y2 = mx[3] * ix + mx[4] * jy + mx[5] * kz + ny / 2;
        z2 = mx[6] * ix + mx[7] * jy + mx[8] * kz + nz / 2;
        // std::cout << "Projecting 3D data with angles: x2=" << x2 << ", y2=" << y2 << ", z2=" <<
        // z2 << std::endl;
        ...
        y = int(floor2(y2));
        z = int(floor2(z2));
        // std::cout << "Projecting 3D data with angles: x=" << x << ", y=" << y << ", z=" << z <<
        // std::endl;
        t = x2 - x;
        u = y2 - y;
        v = z2 - z;
        // std::cout << "Projecting 3D data with angles: t=" << t << ", u=" << u << ", v=" << v <<
        // std::endl;
        ii = x + y * nx + z * xy;
        l = (j + ny / 2) * nx + i + nx / 2;
        // std::cout << "sdata[ii]=" << sdata[ii] << ", sdata[ii+1]=" << sdata[ii+1] << ",
        // sdata[ii+nx]=" << sdata[ii+nx] << std::endl; std::cout << "sdata[ii+nx+1]=" <<
        // sdata[ii+nx+1] << ", sdata[ii+nx*ny]=" << sdata[ii+nx*ny] << ", sdata[ii+xy+1]=" <<
        // sdata[ii+xy+1] << std::endl; std::cout << "sdata[ii+xy+nx]=" << sdata[ii+xy+nx] << ",
        // sdata[ii+xy+nx+1]=" << sdata[ii+xy+nx+1] << std::endl; std::cout << "t=" << t << ", u="
        // << u << ", v=" << v << std::endl; std::cout << "Before: ddata[" << l << "]=" <<
        // ddata[l] << std::endl;
        ddata[l] += trilin(sdata[ii], sdata[ii + 1], sdata[ii + nx], sdata[ii + nx + 1],
                           sdata[ii + nx * ny], sdata[ii + xy + 1], sdata[ii + xy + nx],
                           sdata[ii + xy + nx + 1], t, u, v);
        // std::cout << "After: ddata[" << l << "]=" << ddata[l] << std::endl;
      }
    }
  }
...
void emdata::applyMask(int r, int type, float dx0, float dy0,
                       float dz0) {  // dx0,dy0,dz0 modify the mask center
  int x, y, z, i;                    // relative to default center nx/2,ny/2,nz/2
  float k, rr, m;
  float x1, y1, z1;
  // if (flags&EMDATA_COMPLEX) return;
  if (type == 5 && r <= 0) return;
  if (r <= 0) r = nx / 2;
  nx = header.nx;
  ny = header.ny;
  nz = header.nz;
  getData();
  if (nz == 1) {
    float xc = nx / 2.0 + dx0, yc = ny / 2.0 + dy0;
    switch (type) {
      case 4:
        r = r * r;
        for (y = i = 0; y < ny; y++) {
          for (x = 0; x < nx; x++, i++) {
            if (hypot2(x - xc, y - yc) > r) rdata[i] = 0;
          }
        }
        break;
    }
  } else {
    float xc = nx / 2.0 + dx0, yc = ny / 2.0 + dy0, zc = nz / 2.0 + dz0;
    switch (type) {
      case 4:
        r = r * r;
        for (z = i = 0; z < nz; z++) {
          for (y = 0; y < ny; y++) {
            for (x = 0; x < nx; x++, i++) {
              if (hypot3(x - xc, y - yc, z - zc) < r) continue;
              rdata[i] = 0;
            }
          }
        }
